...
def attention_pool(tensor, pool, hw_shape, has_cls_embed=True, norm=None):
    ...
    if tensor_dim == 4: # [B N L C] BATCH Num_view HW channel
        pass
    elif tensor_dim == 3:
    ...
    # 调整形状 把输入tensor调整为cnn格式并进行池化
    B, N, L, C = tensor.shape
    H, W = hw_shape
    tensor = tensor.reshape(B * N, H, W, C).permute(0, 3, 1, 2).contiguous()

    tensor = pool(tensor) # 卷积或者pool
    
    #  恢复 Transformer 形状
    hw_shape = [tensor.shape[2], tensor.shape[3]] # 返回池化后hw
...
class MultiScaleAttention(nn.Module):
    ...
    def forward(self, x, hw_shape):
        ...
        if self.pool_first:
            ...
            # q k v = [32, 8, 224, 64] [B, heads, hw, ]
       
        # q k v 作注意力池化
        q, q_shape = attention_pool(
            q,
            self.pool_q, # 池化方式pool 或者 conv
            hw_shape,
...
if __name__ == "__main__":
    ...
    model = MultiScaleAttention(dim=64, dim_out=128, input_size = (112, 112), kernel_q=(2, 2), kernel_kv=(2, 2), stride_q=(2, 2), stride_kv=(2, 2))

    x = torch.randn((32, 112*112, 64))
    y = model(x, hw_shape=(112, 112))
    print(y[0].shape)
    print(y[1])